...
export function ResponseErrorInternal(
  ...
  const { stack } = error;
  return {
    apply: (res: Response) => res.status(500).json({ message, error, stack }),
...
export function ResponseSuccessJson<T>(
  o: T,
  status = 200
 ...
 * @param req
 */
export const expressErrorsWrapper = (err: any, req: Request) =>
